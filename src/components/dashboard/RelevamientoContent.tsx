...
export default function RelevamientoContent({ sheetId }: Props) {
  ...
  useEffect(() => {
    async function loadMeta() {
      try {
        const res = await fetch(`/api/sheets?sheetId=${encodeURIComponent(sheetId)}&meta=true`)
        if (!res.ok) {
          const d = await res.json()
          throw new Error(d.error ?? "Error al cargar pestañas")
        }
        const data = await res.json()
        setTabs(data.sheets)
        if (data.sheets.length > 0) setActiveTab(data.sheets[0].title)
      } catch (err) {
  ...
  useEffect(() => {
    ...
    async function loadTab() {
      try {
        setTabLoading(true)
        setTabError(null)
        const range = encodeURIComponent(`${activeTab}!A:Z`)
        const res = await fetch(`/api/sheets?sheetId=${encodeURIComponent(sheetId)}&range=${range}`)
        if (!res.ok) {
          const d = await res.json()
          throw new Error(d.error ?? "Error al cargar datos")
        }
        const data = await res.json()
        ...
        setTabError(err instanceof Error ? err.message : "Error desconocido")
      } finally {
        setTabLoading(false)
      }
    }
